...
<head>
  <title>Preceptores - ADE</title> <link rel="icon" type="image/png" href="../assets/img/ade-icon.png">
  <!-- Required meta tags -->
  <meta charset="utf-8">
	  ...
	  include '../sidebar.php'; ?>
		...
		if ($_SESSION["perms"] == "ADMIN") {
			echo '<td> <font face="Arial"><b>DNI</b></font> </td> 
				  ...
				  <td> <font face="Arial"><b>Acciones</b></font> </td> ';
		}
		echo '</thead>';
		 
		if ($result = $mysqli->query($query)) {
			while ($row = $result->fetch_assoc()) {
				$field1name = $row["id_preceptor"];
				$field2name = $row["nombre"];
				$field3name = $row["apellido"];
				if ($_SESSION["perms"] == "ADMIN") {
					$field4name = $row["dni"];
					$field5name = $row["telefonofijo_p"]; 
					$field6name = $row["telefonocelular_p"]; 
					$field7name = $row["mail"]; 
				}
				...
				if ($_SESSION["perms"] == "ADMIN") {
					echo '<td>'.$field4name.'</td> 
						  ...
						  <td>'.$field7name.'</td> 
						  <td>
							<a href="delete.php?id='.$row['id_preceptor'].'"><i class="material-icons" style="color:white">delete</a>
							<a href="update.php?id='.$row['id_preceptor'].'&nombre='.$row['nombre'].'&apellido='.$row['apellido'].'&dni='.$row['dni'].'&telfijo='.$row['telefonofijo_p'].'&telcelular='.$row['telefonocelular_p'].'&correo='.$row['mail'].'"><i class="material-icons" style="color:white">edit</a>
						</td>
					  </tr>';
				}
			}
